<?php
$page_title = 'Excellence des Paris NBA | Mega Bet 3000';
$page_description = 'Découvrez l\'élite des paris NBA avec Mega Bet 3000. Expertise, comparatifs et conseils exclusifs pour parier avec style et responsabilité.';
include 'header.php';
?>

<!-- Hero Section -->
<section class="hero-section">
    <div class="hero-content">
        <h1>Paris NBA</h1>
        <p>L'excellence des paris sur la NBA avec les meilleures cotes</p>
    </div>
</section>

<!-- Betting Cards -->
<section class="cards-container">
    <div class="betting-card">
        <div class="card-content">
            <div class="card-logo logo-unibet"></div>
            
            <div class="card-bonus">
                <div class="bonus-amount">
                    <span class="bonus-number">100€</span>
                    <span class="bonus-text">de bonus</span>
                </div>
                <div class="bonus-description">Bonus spécial NBA</div>
            </div>
            
            <div class="card-rating">
                <div class="rating-content">
                    <div class="rating-header">
                        <span class="rating-label">Évaluation</span>
                        <span class="rating-percentage">95%</span>
                    </div>
                    <div class="rating-bar">
                        <div class="rating-fill" style="width: 95%;"></div>
                    </div>
                </div>
            </div>
            
            <div class="payment-methods">
                <div class="payment-icon">
                    <svg width="26" height="15" viewBox="0 0 26 15" fill="white">
                        <path d="M8.5 2h9v11h-9z"/>
                        <path d="M1 2h7v11H1z"/>
                        <path d="M18 2h7v11h-7z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="34" height="10" viewBox="0 0 34 10" fill="white">
                        <path d="M0 2h34v6H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="43" height="7" viewBox="0 0 43 7" fill="white">
                        <path d="M0 0h43v7H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="41" height="11" viewBox="0 0 41 11" fill="white">
                        <path d="M0 0h41v11H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="12" viewBox="0 0 35 12" fill="white">
                        <path d="M0 0h35v12H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="14" viewBox="0 0 35 14" fill="white">
                        <path d="M0 0h35v14H0z"/>
                    </svg>
                </div>
            </div>
            
            <a href="https://www.unibet.fr/?utm=n4b8a6s3d7" target="_blank" rel="noopener noreferrer" class="card-button">
                <span class="card-button-text">Débloquez le bonus</span>
            </a>
        </div>
    </div>
    
    <div class="betting-card">
        <div class="card-content">
            <div class="card-logo logo-pmu"></div>
            
            <div class="card-bonus">
                <div class="bonus-amount">
                    <span class="bonus-number">200€</span>
                    <span class="bonus-text">de bonus</span>
                </div>
                <div class="bonus-description">Cotes NBA boostées</div>
            </div>
            
            <div class="card-rating">
                <div class="rating-content">
                    <div class="rating-header">
                        <span class="rating-label">Évaluation</span>
                        <span class="rating-percentage">92%</span>
                    </div>
                    <div class="rating-bar">
                        <div class="rating-fill" style="width: 92%;"></div>
                    </div>
                </div>
            </div>
            
            <div class="payment-methods">
                <div class="payment-icon">
                    <svg width="26" height="15" viewBox="0 0 26 15" fill="white">
                        <path d="M8.5 2h9v11h-9z"/>
                        <path d="M1 2h7v11H1z"/>
                        <path d="M18 2h7v11h-7z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="34" height="10" viewBox="0 0 34 10" fill="white">
                        <path d="M0 2h34v6H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="43" height="7" viewBox="0 0 43 7" fill="white">
                        <path d="M0 0h43v7H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="41" height="11" viewBox="0 0 41 11" fill="white">
                        <path d="M0 0h41v11H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="12" viewBox="0 0 35 12" fill="white">
                        <path d="M0 0h35v12H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="14" viewBox="0 0 35 14" fill="white">
                        <path d="M0 0h35v14H0z"/>
                    </svg>
                </div>
            </div>
            
            <a href="https://www.pmu.fr/?utm=a7s2k9f1l3" target="_blank" rel="noopener noreferrer" class="card-button">
                <span class="card-button-text">Débloquez le bonus</span>
            </a>
        </div>
    </div>
    
    <div class="betting-card">
        <div class="card-content">
            <div class="card-logo logo-netbet"></div>
            
            <div class="card-bonus">
                <div class="bonus-amount">
                    <span class="bonus-number">150€</span>
                    <span class="bonus-text">de bonus</span>
                </div>
                <div class="bonus-description">Paris NBA en live</div>
            </div>
            
            <div class="card-rating">
                <div class="rating-content">
                    <div class="rating-header">
                        <span class="rating-label">Évaluation</span>
                        <span class="rating-percentage">90%</span>
                    </div>
                    <div class="rating-bar">
                        <div class="rating-fill" style="width: 90%;"></div>
                    </div>
                </div>
            </div>
            
            <div class="payment-methods">
                <div class="payment-icon">
                    <svg width="26" height="15" viewBox="0 0 26 15" fill="white">
                        <path d="M8.5 2h9v11h-9z"/>
                        <path d="M1 2h7v11H1z"/>
                        <path d="M18 2h7v11h-7z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="34" height="10" viewBox="0 0 34 10" fill="white">
                        <path d="M0 2h34v6H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="43" height="7" viewBox="0 0 43 7" fill="white">
                        <path d="M0 0h43v7H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="41" height="11" viewBox="0 0 41 11" fill="white">
                        <path d="M0 0h41v11H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="12" viewBox="0 0 35 12" fill="white">
                        <path d="M0 0h35v12H0z"/>
                    </svg>
                </div>
                <div class="payment-icon">
                    <svg width="35" height="14" viewBox="0 0 35 14" fill="white">
                        <path d="M0 0h35v14H0z"/>
                    </svg>
                </div>
            </div>
            
            <a href="https://www.netbetsport.fr/?utm=z8x4c6v9b2" target="_blank" rel="noopener noreferrer" class="card-button">
                <span class="card-button-text">Débloquez le bonus</span>
            </a>
        </div>
    </div>
</section>

<?php include 'footer.php'; ?>